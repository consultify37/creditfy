...
const ImageFormComponent = ({image, setImage, id}: Props) => {
  
  return (
    <div className='flex flex-row mt-8'>
      <div className='flex justify-center items-center rounded-xl bg-white h-[165px] w-[193px]'>
        <input 
          type='file'
          id={id}
          accept="image/*"
        ...
        <label htmlFor={id} className='rounded-full bg-admin-card p-4 flex items-center justify-center cursor-pointer hover:scale-[1.05] transition-all'>
      ...
      { (typeof image === 'string' ? image : ( image ? URL.createObjectURL(image) : '')) != '' ?
        <div className='relative rounded-xl ml-16'>
          <Image 
            src={typeof image === 'string' ? image : ( image ? URL.createObjectURL(image) : '')}
            alt='image-background'
            height={362}
            width={1066}
            className='h-[165px] w-[485px] rounded-xl object-cover'
          />
          <div 
            className='absolute top-4 right-4 cursor-pointer hover:scale-[1.05] transition-all'
