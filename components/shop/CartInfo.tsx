  ...
  return (
    <div className='relative p-8 lg:p-12 py-12 lg:py-16 mt-12 lg:mt-24 bg-secondary rounded-[20px] lg:rounded-[35px] flex flex-col items-center'>
      ...
      <div className='flex flex-col lg:flex-row justify-between gap-16 mt-16 max-w-[960px] w-full'>
        { data.map((item, index) => {
          const rotation = getRotationDeg(index)
          return (
            <div key={item.title} className='flex flex-col items-center'>
              <div className='relative flex items-center justify-center h-[100px] w-[100px]'>
                ...
                <div className={`absolute h-full w-full inset-0 rounded-[20px] ${rotation}`} style={{background: getColorForWhyChoose(index != 1 ? index : 3)[0]}}></div>
              </div>

              <p className='text-[16px] lg:text-[20px] font-semibold text-onSecondary mt-8 text-center'>{item.title}</p>
            </div>
          )})}
      </div>
